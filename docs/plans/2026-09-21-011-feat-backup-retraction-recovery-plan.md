...
本机部分已接入来源撤回与影响清单、完整应用数据备份集、隔离恢复与严格同账本核对、退出可读导出和来源清除清单。当前服务实际使用 `apps/service/src/main.ts` 的维护命令，未新增原计划示意的 `apps/cli/src/maintenance.ts`；使用和代码入口以[接手说明](../implementation/backup-retraction-recovery.md)为准，验证情况见[验证记录](../implementation/backup-retraction-recovery-validation.md)。

上方 O1—O4 保持未勾选：原计划的完整完成依据还包括远端提醒取消与对账、混合派生内容清理、现场事实冲突合并、物理清除及真实资料全量演练。当前没有 relay，也没有可证明已擦除 SQLite 页、旧备份和第三方副本的机制。不得把“撤回”“退出导出”或“清除清单”写成已完成物理删除；无法核对较新现场时保持隔离，不开放旧能力。
