...
目前可以把选定的资料库复制成独立试点，连接插件，收录文本、本地 HTML、静态网页、文档集合、代码快照和 PDF。收录结果先供人检查，批准后才写入试点中的 `KB-Sources`。
...
| 本地 HTML | 一份 `.html` 或 `.htm` 文件的绝对路径；选择“本地 HTML 文件” | 原字节保留；只提取静态文字，图片、脚本和外部资源不会运行或获取 |
